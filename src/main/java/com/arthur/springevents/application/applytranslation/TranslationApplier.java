...
public class TranslationApplier implements ApplyTranslation {
    ...
    public void applyOne(Translation translation) {
        ...
        var affectedDocuments =
                documentService.findAllContainingAndLanguage(
                        translation.getOriginalText(), translation.getTranslatedLanguage());

        affectedDocuments.forEach(
                document -> {
                    log.info("Applying Translation: {} to Document: {}", translation, document);
                    var originalText = document.getText();
                    var translatedText =
                            originalText.replace(
                                    translation.getOriginalText(), translation.getTranslatedText());
